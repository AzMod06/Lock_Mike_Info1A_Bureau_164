...
from APP_BUREAU_164.films.gestion_films_wtf_forms import FormWTFUpdateFilm, FormWTFAddFilm, FormWTFDeleteFilm
    ...
    if request.method == "POST":
        try:
            if form_add_film.validate_on_submit():
                ...
                strsql_insert_film = """INSERT INTO t_personne (id_pers,Prenom_pers) 
                                        VALUES (NULL,%(value_nom_film)s) """
...
def film_update_wtf():
    ...
    try:
        ...
        if form_update_film.validate_on_submit():
            ...
            str_sql_update_nom_film = """UPDATE t_personne SET Prenom_pers = %(value_nom_film)s,
                                                            Nom_pers = %(value_duree_film)s,
                                                            Alias_pers = %(value_description_film)s
                                                            WHERE id_pers = %(value_id_film)s"""
            with DBconnection() as mconn_bd:
            ...
            str_sql_id_film = "SELECT * FROM t_personne WHERE id_pers = %(value_id_film)s"
            ...
            print("data_film ", data_film, " type ", type(data_film), " genre ",
                  data_film["Prenom_pers"])

            # Afficher la valeur sélectionnée dans le champ du formulaire "film_update_wtf.html"
            form_update_film.nom_film_update_wtf.data = data_film["Prenom_pers"]
            form_update_film.duree_film_update_wtf.data = data_film["Nom_pers"]
            form_update_film.description_film_update_wtf.data = data_film["Alias_pers"]
...
def film_delete_wtf():
    ...
    try:
        ...
        if form_delete_film.submit_btn_del_film.data:
            ...
            str_sql_delete_fk_film_genre = """DELETE FROM t_reservation WHERE fk_personne = %(value_id_film)s"""
            str_sql_delete_film = """DELETE FROM t_personne WHERE id_pers = %(value_id_film)s"""
        ...
        if request.method == "GET":
            ...
            str_sql_genres_films_delete = """SELECT * FROM t_personne WHERE id_pers = %(value_id_film)s"""
