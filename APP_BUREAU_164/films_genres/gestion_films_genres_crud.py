...
from APP_BUREAU_164.database.database_tools import DBconnection
from APP_BUREAU_164.erreurs.exceptions import *
    ...
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_films_afficher_data = """SELECT id_pers, Prenom_pers, Nom_pers, Alias_pers,
                                                            GROUP_CONCAT(Num_bureau) as GenresFilms FROM t_reservation
                                                            RIGHT JOIN t_personne ON t_personne.id_pers = t_reservation.fk_personne
                                                            LEFT JOIN t_bureau ON t_bureau.id_bureau = t_reservation.fk_bureau
                                                            GROUP BY id_pers"""
                if id_film_sel == 0:
                    ...
                    strsql_genres_films_afficher_data += """ HAVING id_pers= %(value_id_film_selected)s"""
...
def edit_genre_film_selected():
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_afficher = """SELECT id_bureau, Num_bureau, Salle_bureau FROM t_bureau ORDER BY id_bureau ASC"""
            ...
            lst_data_film_selected = [item['id_pers'] for item in data_genre_film_selected]
            ...
            lst_data_genres_films_non_attribues = [item['id_bureau'] for item in data_genres_films_non_attribues]
            ...
            lst_data_genres_films_old_attribues = [item['id_bureau'] for item in data_genres_films_attribues]
            ...
            lst_data_genres_films_non_attribues = [item['Num_bureau'] for item in data_genres_films_non_attribues]
...
def update_genre_film_selected():
    if request.method == "POST":
        try:
            ...
            strsql_insert_genre_film = """INSERT INTO t_reservation (id_reservation, fk_bureau, fk_personne)
            ...
            strsql_delete_genre_film = """DELETE FROM t_reservation WHERE fk_bureau = %(value_fk_genre)s AND fk_personne = %(value_fk_film)s"""
...
def genres_films_afficher_data(valeur_id_film_selected_dict):
    ...
    try:
        ...
        strsql_film_selected = """SELECT id_pers, Prenom_pers, nom_pers, Alias_pers, GROUP_CONCAT(id_bureau) as GenresFilms FROM t_reservation
                                        INNER JOIN t_personne ON t_personne.id_pers = t_reservation.fk_personne
                                        INNER JOIN t_bureau ON t_bureau.id_bureau = t_reservation.fk_bureau
                                        WHERE id_pers = %(value_id_film_selected)s"""

        strsql_genres_films_non_attribues = """SELECT id_bureau, Num_bureau, Salle_bureau FROM t_bureau WHERE id_bureau not in(SELECT id_bureau as idGenresFilms FROM t_reservation
                                                    INNER JOIN t_personne ON t_personne.id_pers = t_reservation.fk_personne
                                                    INNER JOIN t_bureau ON t_bureau.id_bureau = t_reservation.fk_bureau
                                                    WHERE id_pers = %(value_id_film_selected)s)"""

        strsql_genres_films_attribues = """SELECT id_pers, id_bureau, date_reserv FROM t_reservation
                                            INNER JOIN t_personne ON t_personne.id_pers = t_reservation.fk_personne
                                            INNER JOIN t_bureau ON t_bureau.id_bureau = t_reservation.fk_bureau
                                            WHERE id_pers = %(value_id_film_selected)s"""
